...
void CalcNLocTri(map<int, int> *a_mat, string outfile, double &tri_num_ns, double &tri_num_emp){
	...
	tri_num_emp = (double)tri_num * q_inv_11 + (double)ed2_num * q_inv_21 + (double)ed1_num * q_inv_31 + (double)non_num * q_inv_41;
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) {
	    printf("Usage: %s [EdgeFile (in)] ([#nodes (default: -1)] [epsilon (default: 1)] [#itr(-1) (default: 1)] \n\n", argv[0]);
	...
	outdir = EdgeFile.substr(0, i+1);
	for(i=0;i<3;i++){
		if(fix_perm) outfile = outdir + "res_n_" + to_string(NodeNum) + "_alg_" + Alg + "_eps_" + Eps_s + "_itr_" + to_string(ItrNum) + "-1.csv";
		else outfile = outdir + "res_n_" + to_string(NodeNum) + "_alg_" + Alg + "_eps_" + Eps_s + "_itr_" + to_string(ItrNum) + ".csv";
		fp = FileOpen(outfile, "w");
	...
	for(itr=0;itr<ItrNum;itr++){
		...
		// Calculate #triangles
		if(NodeNum <= 10000) CalcNLocTri(a_mat, outfile, tri_num_ns, tri_num_emp);
		else tri_num_ns = 0.0;
